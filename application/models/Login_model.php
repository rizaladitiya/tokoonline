...
class Login_model extends CI_Model
{
	...
	function aktivasi_user($id)
    {
		...
		$this->m_db->edit_row($this->tbl_login,$d,$s);
	}
	...
	function user_android_login($nomer,$password)
    {
		$array=array();
		$s=array(
		'nomer'=>$nomer,
		...
		if($this->m_db->is_bof($this->tbl_login,$s)==FALSE)
		{
			...
			{
				$akses=$this->m_db->get_row($this->tbl_login,$s2,'akses');
				$status=$this->m_db->get_row($this->tbl_login,$s2,'status');
				$nama=$this->m_db->get_row($this->tbl_login,$s2,'nama');
				$this->session->set_userdata($this->tbl_sesi,array(
				'nomer'=>$nomer,
				'akses'=>$akses,
				));
				...
				$this->m_db->edit_row($this->tbl_login,$d,$s2);
				$array=array('success'=>true,'nomer'=>$nomer,'nama'=>$nama,'akses'=>$akses,'status'=>$status);
			}else{
	...
	function user_daftar_android($nama,$nomer,$password,$email)
	{	
		$array=array();
		$s=array(
		'username'=>$username,
	...
	function user_daftar($nama,$username,$password,$email,$akses)
	{
		$s=array(
		'username'=>$username,
		);
	...
	function user_edit($userid,$nama,$username,$password='',$email,$akses)
	{
		$s=array(
		'user_id'=>$userid,
		);
