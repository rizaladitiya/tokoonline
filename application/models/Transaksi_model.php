...
class Transaksi_model extends CI_Model
{
	...
	function penjualan_cod($penjualanID,$lat,$lng,$alamat,$proses)
	{
		$d=array(
			...
			$this->m_db->add_row('penjualan_cod',$d);
	}
	function order_ojek($pelangganID,$lat,$lng,$alamat)
	{
		$d=array(
			'user'=>$pelangganID,
			'lat'=>$lat,
			'lng'=>$lng,
			'alamat'=>$alamat,
			'jemput'=>date('Y-m-d H:i:s')
			);
			$this->m_db->add_row('ojek',$d);
			return $this->m_db->last_insert_id();
	}
	function order_laundry($pelangganID,$lat,$lng,$alamat)
	{
		$d=array(
			'user'=>$pelangganID,
			'lat'=>$lat,
			...
			$this->m_db->add_row('laundry',$d);
			return $this->m_db->last_insert_id();
	}
	function penjualan_add($pelangganID,$total,$kurir,$pelayanan,$ongkir,$produk=array(),$berat,$diskon,$status)
	{
		$dx=array();
		if(!empty($produk))
		{
			...
			$tahun=date("y");
			$bulan=romanic_number(date("m"));
			$tanggal=date("d");
			$invoice="INV/".$tahun."/".$bulan."/".$tanggal."/".strtotime($tgl);
			$d=array(
