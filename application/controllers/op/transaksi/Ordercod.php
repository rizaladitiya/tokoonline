...
class Ordercod extends CI_Controller {
	...
	function index()
	{
		...
		$this->load->view(akses().'/transaksi/orderan/orderviewcod',$d);
	...
	function detail()
	{
		...
		$this->load->view('tema/header',$meta);
		$d['data']=$this->m_db->get_data('penjualan',$s);
		$d['map']=$this->m_db->get_data('penjualan_cod',$s);
		$this->load->view(akses().'/transaksi/orderan/detailview',$d);
		$this->load->view(akses().'/transaksi/orderan/ordermapview');
		$this->load->view('tema/footer');
	}
