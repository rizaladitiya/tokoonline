...
class Produk extends CI_Controller
{
	...
	function index()
	{
		$meta['judul']=baca_konfig('nama-aplikasi')." - ".baca_konfig('company-name');
	...
	function kurirdata()
	{
		...
		if($kurir=='cod'){
			$data['data']=$this->m_db->get_data('cod',array());
			$this->load->view('html/page/codview',$data);
		} else {
	...
	function selesaicod()
	{
		...
		if(akses()!="member")
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				//var_dump($_POST);
				$ext=$this->mod_transaksi->penjualan_add($pelanggan,$total,$kurir,$service,$ongkir,$produk,$berat,$diskon,'konfirmasi');
				if($ext['status']==TRUE)
				{					
					$penjualanID=$ext['penjualanid'];
					$this->mod_transaksi->penjualan_cod($penjualanID,$lat,$lng,$alamat,'packing');
					$this->cart->destroy();
					$body="Order COD Baru Harap Segera dikirim<br>http://http://maps.google.com/?q=$lat,$lng";
					$fromname='Order Baru dari Pelanggan';
					$tos=$this->m_db->get_data('userlogin',array('akses'=>'op'));
					foreach($tos as $row){
						$to[] = $row->email;
					}
					//$to=array("ryhorn@gmail.com");
					$subject = 'Order Baru dari Pelanggan';
					//$cekemail = sendEmail($fromname,$to,$subject,$body);
				...
				$meta['judul']="Masukkan Lokasi Pengiriman | ".baca_konfig('nama-aplikasi');
				$meta['judulhalaman']="Lokasi Pengiriman";
				$meta['homepage']=FALSE;
				$this->load->view('html/header',$meta);
				$d['data']=array('pelangganid'=>$pelanggan,'total'=>$total,'kurir'=>$kurir,'service'=>$service,'ongkir'=>$ongkir,'produk'=>$produk,'berat'=>$berat,'diskonnilai'=>$diskon);
				$this->load->view('html/page/inputcodview',$d);
				$this->load->view('html/footer');
			}			
		}	
	}
	
	function selesaiojek()
	{
		//$this->load->library('PHPMailerAutoload');
		if(akses()!="member")
		{
			...
			redirect(base_url().'member/login?ref='.$ref);
				...
				//var_dump($_POST);
				$ext=$this->mod_transaksi->order_ojek($pelanggan,$lat,$lng,$alamat);
				if($ext!=0)
				{					
					$body="Permintaan jemput ojek<br>http://http://maps.google.com/?q=$lat,$lng";
					$fromname='Ojek dari Pelanggan';
					$tos=$this->m_db->get_data('userlogin',array('akses'=>'op'));
					foreach($tos as $row){
						$to[] = $row->email;
					}
					$subject = 'Order Baru dari Pelanggan';
					//$cekemail = sendEmail($fromname,$to,$subject,$body);
					redirect(base_url().'produk/histori/ojek/'.$ext);
	...
	function selesailaundry()
	{
		...
		if(akses()!="member")
		{
			...
			redirect(base_url().'member/login?ref='.$ref);
				...
				//var_dump($_POST);
				$ext=$this->mod_transaksi->order_laundry($pelanggan,$lat,$lng,$alamat);
				if($ext!=0)
				{					
					$body="Permintaan jemput laundry<br>http://http://maps.google.com/?q=$lat,$lng";
					$fromname='Laundry dari Pelanggan';
					$tos=$this->m_db->get_data('userlogin',array('akses'=>'op'));
					foreach($tos as $row){
						$to[] = $row->email;
					}
	...
	function orderojek()
	{
		if(akses()!="member")
		{
			...
			redirect(base_url().'member/login?ref='.$ref);
				...
				$meta['judul']="Masukkan Lokasi Anda | ".baca_konfig('nama-aplikasi');
				$meta['judulhalaman']="Lokasi Penjemputan Pelanggan";
				$meta['homepage']=FALSE;
				$this->load->view('html/header',$meta);
				$this->load->view('html/page/inputojekview');
				$this->load->view('html/footer');
		}
	}
	function orderlaundry()
	{
		if(akses()!="member")
		{
			...
			redirect(base_url().'member/login?ref='.$ref);
				...
				$meta['judul']="Masukkan Lokasi Jemput Laundry | ".baca_konfig('nama-aplikasi');
				$meta['judulhalaman']="Lokasi Jemput Laundry";
				$meta['homepage']=FALSE;
				$this->load->view('html/header',$meta);
				$this->load->view('html/page/inputlaundryview');
				$this->load->view('html/footer');
		}
	}
	function selesai()
	{
		if(akses()!="member")
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				//var_dump($_POST);
				$ext=$this->mod_transaksi->penjualan_add($pelanggan,$total,$kurir,$service,$ongkir,$produk,$berat,$diskon,'draft');
				if($ext['status']==TRUE)
				{					
					$penjualanID=$ext['penjualanid'];
					$this->cart->destroy();
